...
- `CanvasCardContent.tsx` renders both spatial cards and the full reading panel. `mine-canvas.css` owns canvas styles.
...
5. For new functionality, extend the discriminated data type and the shared React content renderer. Keep cards useful in both the spatial preview and the reading panel. Keep new interactions compatible with the drag handle and mobile scrolling.
