...
- Three.js 低多边形独立桌椅构图：书桌、电脑、桌面文件架与台灯；桌面平放的 iPad连接个人画布。没有墙体和可见地板，仅保留透明接影。支持约 195° 水平拖动范围及更宽的俯仰视角；部件悬停或触摸按下时轻微高亮，提示文字无底色。场景铺满首页，底部直接叠放 `JUSTIN / PERSONAL SPACE` 署名，首页不显示 Dock。
...
- 电脑和 iPad共用约 1.8 秒的空间过渡：物件固定，摄像机先转到屏幕正面，再沿屏幕法线靠近；靠近过程中实时页面贴着屏幕渐显并接管全屏，返回时逆向收回。电脑进入 `/os`，iPad 进入 `/canvas`，复用原组件和数据。减少动态效果模式直接进入。
