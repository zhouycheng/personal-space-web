...
- 首页为 Three.js 工作室，五个物件连接 OS、作品集、介绍、人生系统与公开链接。仅保留底部居中署名；备用入口在键盘聚焦或加载失败时显示，首页隐藏 Dock。背景与灯光随访客本地时间自动变化。
- 首页使用 `room`、`entering`、`desktop`、`returning` 状态，定义见 `../CONTEXT.md`；仅在当前标签页恢复房间或桌面稳定态。
