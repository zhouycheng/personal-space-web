...
- 桌面平放的 iPad打开“我的画布”，桌面文件架打开悬浮卡片作品集；电脑和 iPad始终固定在场景中，共用约 1.8 秒的镜头过渡：先转正，再沿屏幕法线靠近，实时页面在靠近过程中贴屏渐显并接管全屏，返回时逆向收回。个人画布保留原编辑器、鉴权和数据接口。
- 水平拖动范围约 195°，支持更宽的俯仰视角；悬停与触摸按下仅轻微高亮当前部件，提示文字取消底色。场景延伸至底部，署名直接叠放，桌面字号 16px、窄屏 13px。
