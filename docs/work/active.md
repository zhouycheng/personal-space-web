...
- `src/components/app/studioAppRuntime.ts`：URL 驱动的 History API 导航、面板和空间动画；返回复用已有首页记录。
...
- 画板打开“我的画布”，书架打开悬浮卡片作品集；电脑和画板始终固定在房间中，共用约 1.45 秒的摄像机靠近/后退过渡，靠近末尾淡入可操作页面。个人画布保留原编辑器、鉴权和数据接口。
