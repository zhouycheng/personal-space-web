...
`/journal` 先进入合拢观察，点击封面或“打开日记”后恢复有效的本机书签，否则打开最新文章；独立文章 URL 直接阅读，优先于其他文章的书签。实体书按文章日期正序编排，每篇另起一页。重新分页后优先以文章和段落锚点恢复位置。

可用宽度扣除 80px 后达到 680px、可用高度扣除 160px 后达到 480px 时展示双页。双页翻动右侧纸张：正面为当前右页，背面为下一展开的左页。其余窗口单页逐面阅读，旋转屏幕保持当前内容。翻页、观察和开合不增加浏览器历史；独立文章 URL 由浏览器历史管理。

合拢时拖动物件可自由旋转，松手即停。打开后水平旋转限制为 ±25°、垂直为 ±15°。页角拖动支持回弹，翻动期间不累计输入；其余书本区域拖动调整角度。手势在按下时确定，移动超过 6px 不会误触封面、链接或图片。

滚轮、双指捏合与加减按钮直接缩放 3D 书本，放大后拖动平移。“摆正”同时复位角度、缩放和平移。页内链接和图片通过清单坐标映射响应，只有文章插图使用图片对话框；链接与图片优先响应，普通页面点击翻页。

“合拢”回到物件观察，“返回首页”合拢后归位。Esc 依次关闭图片或文字阅读，之后合拢，再退出。阅读时方向键翻页；全部按钮可通过键盘访问。入场、开合与翻页由独立状态驱动，路由取消旧动画和手势。
...
书本复用工作室的 WebGLRenderer、相机和按需渲染循环。外壳为深海军蓝哑光硬封面，配小号哑银标题、暖白纸芯和素色内衬，不设装饰边框、绑带或书签飘带。抽屉模型与阅读外壳共用 `journalAppearance.ts` 配色。纸面参与光照且有轻微弧度；拿起、观察和阅读沿用工作室的实时光照与背景，不增加遮罩或书本补光；阅读控件随环境切换深浅文字。只缓存当前与附近两组展开面的纹理；放回抽屉释放纹理，场景销毁时释放事件、材质与几何资源，页面隐藏停止渲染。减少动态效果时直接切换稳定状态，仍支持用户直接拖动。
...
浏览器测试默认连接 `http://127.0.0.1:4321`；用 `JOURNAL_TEST_URL` 指向隔离的测试服务，`PLAYWRIGHT_CHANNEL=chrome` 可选择已安装的 Chrome。分页测试使用临时内容、资源及输出目录，覆盖 6–10 页中文混排、缓存及缺失图片失败后的旧清单保护。双页翻动测试在浏览器响应中注入六页清单，不改文章源文件。
...
点击左页上一页、右页下一页；手机单页按左右半页区分方向。链接与图片优先响应，拖动不触发点击翻页。首尾只禁用对应方向，保留页角拖动与键盘操作。返回首页与文件夹、画布统一放在左上角；没有外置目录，文章直达与文字阅读保留。
