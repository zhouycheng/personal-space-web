...
- 保持 TTL 使过期状态消失，
- 避免保存历史记录，
- 本站应用名称到显示文案的映射位于 `src/data/selectors/activityText.ts`；Kit 状态组件只消费快照。

当前 Astro 实现：

- `scripts/activity-monitor.mjs` 加载 `.env.local` 或 `.env`，使用 `/usr/bin/osascript` 读取前台应用，并发送 Bearer Token 更新。
- `src/pages/api/activity/update.ts` 在更新内存存储前验证 token 和载荷。
- `src/data/stores/activity/store.ts` 仅保留最新支持的应用快照，并在 `25000` ms 后过期。
- `src/infrastructure/server/activityStream.ts` 通过 SSE 流式推送快照，每 `15000` ms 发送心跳注释。
