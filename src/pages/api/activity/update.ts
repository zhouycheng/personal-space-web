...
function parseActivityPayload(body: unknown): ActivityUpdatePayload | null {
  ...
  const appName = typeof rawAppName === "string" && rawAppName.trim().length > 0 ? rawAppName.trim().slice(0, 120) : null;
  const rawObservedAt = candidate.observedAt;
  const observedAt = typeof rawObservedAt === "number" && Number.isFinite(rawObservedAt) ? rawObservedAt : undefined;
  const rawSessionId = candidate.sessionId;
  const sessionId = typeof rawSessionId === "string" && rawSessionId.trim().length > 0 ? rawSessionId.trim().slice(0, 120) : null;
  return { appName, state, observedAt, sessionId };
}
