...
import { BOOK_HEIGHT as H, createJournalBookGeometry } from "./journalBookGeometry";
import { spreadFor, turnFaces } from "../../../application/journal/book-state";
import type { JournalManifest, JournalRegion, BookReport } from "../../../contracts/journal";
import { clamp, constrainReading, journalSingle, type JournalPhase } from '../../../application/journal/inspection';
...
export function createJournalBook(renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.PerspectiveCamera, requestDraw: () => void) {
  ...
  } = createJournalBookGeometry(scene);
  ...
  const canInteract=()=>active&&travelAmount===1&&!opening&&interactionEnabled;
  ...
  return {
    root,
    setInteractionEnabled(value:boolean){interactionEnabled=value;if(!value)releasePointers();},
