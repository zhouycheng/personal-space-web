...
import type { JournalManifest, ReadingAnchor, JournalRegion, BookReport } from "../../../contracts/journal";
import { journalSlug, resolveReadingPage, spreadFor } from "../../../application/journal/book-state";
import type { JournalPhase } from '../../../application/journal/inspection';

export function createJournalReader(root:HTMLElement, scene:()=>ScenePort|undefined, navigate:(path:string)=>void) {
  ...
  async function enter(path:string,duration:number){
    ...
    textSlug=journalSlug(new URL(path,location.origin).pathname)??'';
    page=requestedPage(path);syncText();
    if(!book.pages.length){status.hidden=false;status.textContent=book.articles.length?'书页尚未生成，正文仍可阅读。':'日记尚未写下第一篇。';mode.disabled=true;setMode(true);return;}
    const current=scene();
  ...
  return {book,enter,fallback,
    select(path:string){textSlug=journalSlug(new URL(path,location.origin).pathname)??'';page=requestedPage(path);scene()?.setJournalPage(page);if(book.pages.length&&textSlug)void scene()?.openJournal(true);syncText();text.scrollTop=0;},
